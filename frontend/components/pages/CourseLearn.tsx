...
function CooldownTimer({ seconds: initial }: { seconds: number }) {
  ...
  return (
    <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
      <p className="text-xs text-amber-700 font-medium">Cooldown Aktif</p>
      <p className="text-xs text-amber-600 mt-0.5 font-mono">Tunggu {parts.join(' ')} lagi</p>
    </div>
  );
}
...
export default function CourseLearnPage({ basePath = '/courses' }: { basePath?: string }) {
  ...
  // Timer effect: resume from localStorage (biar tahan refresh/new tab), fallback ke backend
  useEffect(() => {
  ...
  // Stop timer otomatis saat waktu belajar minimum terpenuhi
  useEffect(() => {
  ...
  // Save timer saat ini ke server (dipanggil sebelum pindah lesson / close)
          ...
          <div className="flex items-center gap-4 p-6 bg-gray-50 rounded-lg">
            ...
            <div>
              ...
              {currentLesson.file_url && (
                <a href={currentLesson.file_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm">
                  Download/View File
                </a>
              )}
            </div>
          ...
          <div className="flex items-center gap-4 p-6 bg-gray-50 rounded-lg">
            ...
            <div>
              ...
              {currentLesson.external_url && (
                <a href={currentLesson.external_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm">
                  {currentLesson.external_url}
                </a>
              )}
            </div>
      ...
      <div className="flex items-center justify-center py-20">
        <Card>
          <CardContent className="p-12 text-center">
            <p className="text-gray-600 mb-4">Anda belum mendaftar kursus ini.</p>
  ...
  return (
    ...
    <div className="flex">
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'w-80' : 'w-0'} bg-white border-r border-gray-200 transition-all duration-300 overflow-hidden flex flex-col flex-shrink-0`}>
        <div className="p-4 border-b border-gray-200">
          ...
          <div className="mt-2">
            ...
            <p className="text-xs text-gray-600 mt-1">{progress.completed_lessons}/{progress.total_lessons} selesai</p>
            ...
            {(() => {
              ...
              return groups.map((group) => (
                <div key={group.module_id}>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider px-3 py-1">{group.module_title}</p>
                        ...
                        >
                          <div className="mt-0.5">
                            {lesson.is_completed ? (
                              ...
                              <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center text-[10px] font-medium ${
                                globalIndex === currentLessonIndex ? 'border-blue-500 text-blue-600' : 'border-gray-300 text-gray-400'
                              }`}>
                                {globalIndex + 1}
                              </div>
                            )}
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-xs truncate ${
                              lesson.is_unlocked === false ? 'text-gray-400' :
                              globalIndex === currentLessonIndex ? 'font-medium text-blue-700' : 'text-gray-700'
                            }`}>
                              {lesson.title}
                            </p>
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[10px] text-gray-500">{getContentTypeLabel(lesson.content_type)}</span>
                              ...
                              {lesson.quiz_score !== null && lesson.quiz_score !== undefined && lesson.content_type === 'quiz' && (
                                <span className={`text-[10px] font-medium ${lesson.quiz_passed ? 'text-green-600' : 'text-red-500'}`}>
                                  {lesson.quiz_score}%
                                </span>
                              )}
                            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-4">
          ...
          {currentLesson && currentLesson.content_type !== 'quiz' && currentLesson.is_unlocked !== false && (
            <div className="flex items-center gap-2 text-sm">
              <span className="text-gray-400">Belajar</span>
        ...
        {currentLesson ? (
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              {progress.progress_percentage >= 100 && progress.certificate && (
                <Card className="border-green-300 bg-green-50">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-green-600 mb-2">Anda telah menyelesaikan seluruh materi.</p>
                    <div className="inline-block bg-white rounded-lg border border-green-200 p-4 mb-4">
                      <p className="text-sm text-gray-500">Nomor Sertifikat</p>
                      <p className="font-mono font-bold text-gray-800">{progress.certificate.certificate_number}</p>
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm text-gray-500">{currentLesson.module_title}</span>
                  <span className="text-gray-300">/</span>
                  <Badge variant="outline" className="text-xs">
                    {getContentTypeLabel(currentLesson.content_type)}
                  </Badge>
                  <span className="text-xs text-gray-500">{currentLesson.duration_minutes} menit</span>
                  {currentLesson.content_type !== 'quiz' && currentLesson.is_unlocked !== false && (
                ...
                <CardContent className="p-6 relative">
                  {currentLesson.is_unlocked === false ? (
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                      ...
                      <h3 className="text-lg font-semibold text-gray-600">Materi Terkunci</h3>
                ...
                currentLesson.quiz_score !== null && currentLesson.quiz_score !== undefined ? (
                  <Card className={currentLesson.quiz_passed ? 'border-green-200' : 'border-red-200'}>
                    <CardContent className="p-6 text-center">
                      ...
                      <p className={`text-2xl font-bold mt-2 ${currentLesson.quiz_passed ? 'text-green-600' : 'text-red-500'}`}>
                      ...
                      {!currentLesson.quiz_passed && currentLesson.quiz_max_attempts !== null && (
                        <div className="mt-3 pt-3 border-t border-gray-100">
                          {currentLesson.quiz_cooldown_remaining > 0 ? (
                          ...
                          ) : currentLesson.quiz_max_attempts === -1 || currentLesson.quiz_attempts_count < currentLesson.quiz_max_attempts ? (
                            ...
                            <p className="text-xs text-red-600 font-medium">Batas percobaan telah habis</p>
                          )}
                        </div>
                      )}
                      <Button
                  ...
                  <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg text-center">
                    ...
                    <p className="text-sm text-amber-700 font-medium">Kuis Belum Tersedia</p>
                    <p className="text-xs text-amber-600 mt-1">Soal kuis belum ditambahkan oleh instruktur.</p>
                  </div>
                  ...
                  <div className="text-center p-4 bg-red-50 border border-red-200 rounded-lg">
                    ...
                    <p className="text-xs text-red-600 mt-1">Anda telah mencapai batas maksimal percobaan ({currentLesson.quiz_max_attempts}x).</p>
                  </div>
                ) : currentLesson.is_unlocked === false ? (
                  ...
                  <Card className="border-blue-200">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-blue-700">Kuis Siap Dikerjakan</p>
                ...
                <div className="flex gap-2">
                  {currentLesson.content_type !== 'quiz' && (
                    <Button
                      ...
                      className={currentLesson.is_completed ? 'text-green-600 border-green-300' : ''}
                    >
                  ...
                  {canBypassTimer && currentLesson.content_type !== 'quiz' && !currentLesson.is_completed && currentLesson.duration_minutes > 0 && (
                    <Button
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <p className="text-gray-500 mb-4">Kursus ini belum memiliki pelajaran.</p>
