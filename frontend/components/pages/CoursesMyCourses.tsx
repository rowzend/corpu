...
const statusConfig: Record<string, { label: string; style: string }> = {
    active: { label: 'Aktif', style: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' },
    completed: { label: 'Selesai', style: 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' },
    dropped: { label: 'Berhenti', style: 'bg-muted text-muted-foreground' },
};
                ...
                <div className="relative z-10">
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            ...
                            className="inline-flex items-center gap-2 bg-card text-indigo-700 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 px-5 py-2.5 rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl"
                        >
                ...
                <div className="space-y-3">
                    {filtered.map((enrollment) => {
                        ...
                        return (
                            <div
                                key={enrollment.id}
                                className="bg-card rounded-xl shadow-sm border border-border hover:shadow-md hover:border-indigo-100 dark:hover:border-indigo-800 transition-all group border-l-4 border-l-transparent hover:border-l-indigo-500"
                            >
                                <div className="p-5">
                                    <div className="flex flex-col sm:flex-row gap-4">
                                        {enrollment.course_thumbnail && (
                                            <div className="w-full sm:w-40 h-28 bg-muted rounded-xl overflow-hidden flex-shrink-0">
                                                <img
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    {enrollment.course_description && (
                                                        <p className="text-sm text-muted-foreground dark:text-muted-foreground line-clamp-1 mb-3"
                                            ...
                                            <div className="max-w-md mb-2">
                                                <div className="flex justify-between text-sm text-muted-foreground dark:text-muted-foreground mb-1">
                                                    <span>Progress</span>
                                                    <span className="font-medium text-indigo-600 dark:text-indigo-400">{enrollment.progress_percentage}%</span>
                                                </div>
                                            ...
                                            {enrollment.course_duration_minutes > 0 && (
                                                <div className="flex items-center gap-2 text-xs text-muted-foreground mb-4">
                                                    ...
                                                    <span className="text-muted-foreground">{enrollment.completed_duration_minutes}m</span>
                                                    <span>selesai</span>
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button
                                                    onClick={() => router.push(`${basePath}/${enrollment.course_slug}`)}
                                                    className="inline-flex items-center gap-1.5 px-4 py-2 text-sm text-muted-foreground hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl transition-colors"
                                                >
