      ...
      <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-16 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Kursus ASN Academy</h1>
            <p className="text-xl text-blue-100 mb-8">
              Tingkatkan kompetensi Anda dengan berbagai program pelatihan yang dirancang khusus untuk ASN
            </p>

            <div className="grid grid-cols-3 gap-6 max-w-2xl mx-auto">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                <div className="text-3xl font-bold">{courses.length}+</div>
                <div className="text-sm text-blue-100">Kursus</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
        ...
        <div className="flex gap-8">
              ...
              <div className="mb-6">
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-2.5 pl-10 text-sm border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-50"
                  />
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                  <label className="flex items-center cursor-pointer group">
                    <input
                      ...
                      className="w-4 h-4 text-blue-600 focus:ring-blue-500"
                    />
                    <span className="ml-2 text-sm text-gray-700 group-hover:text-blue-600">Semua Kategori</span>
                  </label>
                  {categories.map((cat) => (
                    <label key={cat} className="flex items-center cursor-pointer group">
                      <input
                        ...
                        className="w-4 h-4 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="ml-2 text-sm text-gray-700 group-hover:text-blue-600">{cat}</span>
                    </label>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center cursor-pointer group">
                    <input
                      ...
                      className="w-4 h-4 text-blue-600 focus:ring-blue-500"
                    />
                    <span className="ml-2 text-gray-700 group-hover:text-blue-600">Semua</span>
                  </label>
                  {[5, 4, 3].map((rating) => (
                    <label key={rating} className="flex items-center cursor-pointer group">
                      <input
                        ...
                        className="w-4 h-4 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="ml-2 flex items-center text-gray-700 group-hover:text-blue-600">
          ...
          <div className="flex-1">
            <div className="mb-8 flex items-center justify-between">
              <div>
                <h2 className="text-3xl font-bold text-gray-900 mb-2">Daftar Kursus</h2>
                <p className="text-gray-600">Menampilkan <span className="font-semibold text-blue-600">{filteredCourses.length}</span> kursus</p>
              </div>
            ...
            <div className="space-y-6">
              {paginatedCourses.map((course, index) => (
                <div
                  key={course.id}
                  className="group bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all duration-500 overflow-hidden transform hover:-translate-y-1 cursor-pointer"
                    ...
                    <div className="flex-1 p-6 md:p-8 flex flex-col justify-between">
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                        <div className="flex items-center gap-1">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <span key={i} className={`text-xl ${i < Math.round(course.rating_avg || 0) ? 'text-yellow-400' : 'text-gray-300'}`}>
                              ★
                            </span>
                          ))}
                          ...
                          <span className="text-xs text-gray-400 ml-1">({course.rating_count || 0})</span>
                        </div>
                        <div className="flex items-center gap-4">
                          <span className="text-sm text-gray-500">
                            {course.lesson_count || 0} pelajaran
                          </span>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-10">
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-sm font-medium border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                >
                  Sebelumnya
                </button>
            ...
            {filteredCourses.length === 0 && (
              <div className="text-center py-16 bg-white rounded-2xl shadow-md">
                ...
                <p className="text-gray-500 text-lg mb-6">Tidak ada kursus yang sesuai dengan filter Anda</p>
                <button
                  onClick={() => {
                    setSearchTerm('');
