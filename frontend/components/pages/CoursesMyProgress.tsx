...
export default function CoursesMyProgress({ basePath = '/courses' }: { basePath?: string }) {
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
                    <div className="p-5">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h2 className="font-semibold text-card-foreground">Progress Terbaik</h2>
                        </div>
                        <div className="flex items-center justify-between">
                            <div className="flex-1 min-w-0">
                                <p className="font-medium text-card-foreground truncate">{topCourse.course_title}</p>
                                <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                                    ...
                                    <Badge className={
                                        topCourse.status === 'completed' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 border-0' :
                            ...
                            <div className="text-right flex-shrink-0 ml-4">
                                <p className="text-3xl font-bold text-emerald-600 dark:text-emerald-400">{topCourse.progress_percentage}%</p>
                            </div>
                        </div>
                        <div className="mt-3">
            ...
            {/* Detail Progress */}
            <h2 className="text-lg font-semibold text-card-foreground">Detail Progress per Kursus</h2>

            {enrollments.length === 0 ? (
                <div className="bg-card rounded-xl shadow-sm border border-border p-12 text-center">
                    <div className="w-16 h-16 mx-auto mb-4 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center">
                        <BarChart3 className="w-8 h-8 text-emerald-500 dark:text-emerald-400" />
                    </div>
                    <h3 className="text-lg font-semibold text-card-foreground mb-2">Belum ada data</h3>
                    <p className="text-muted-foreground mb-6">Anda belum terdaftar di kursus manapun</p>
                    <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {enrollments.map((enrollment) => (
                        <div
                            key={enrollment.id}
                            className="bg-card rounded-xl shadow-sm border border-border hover:shadow-md hover:border-emerald-100 dark:hover:border-emerald-800 transition-all"
                        >
                            <div className="p-5">
                                ...
                                <div className="mb-3">
                                    <div className="flex justify-between text-sm text-muted-foreground mb-1">
                                        <span>Progress</span>
                                        <span className="font-semibold text-emerald-600 dark:text-emerald-400">{enrollment.progress_percentage}%</span>
                                    </div>
                                ...
                                {enrollment.course_duration_minutes > 0 && (
                                    <div className="flex items-center gap-3 text-xs text-muted-foreground mb-3">
                                        <span className="flex items-center gap-1">
                                            ...
                                            <span className="text-muted-foreground font-medium">{enrollment.completed_duration_minutes}m</span>
                                            <span>selesai</span>
                                        </span>
