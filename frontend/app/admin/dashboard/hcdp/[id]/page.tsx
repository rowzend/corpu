...
export default function EditHCDPPage() {
    ...
    useEffect(() => {
        const fetchProgram = async () => {
            ...
            try {
                setLoading(true);
                const res = await getHCDPProgram(id);
                ...
                setFormData({
                    ...
                    duration: p.duration || '',
                    location: p.location || '',
                    max_participants: p.max_participants || 30,
                ...
                showError(handleApiError(err), 'Gagal Memuat Program');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showError(handleApiError(error), 'Gagal Memperbarui Program');
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async () => {
        const confirmed = await showConfirm(
            `Yakin ingin menghapus program "${formData.title}"?`,
            'Hapus Program',
            'Hapus',
            'Batal'
        );
        ...
        try {
            ...
            showError(handleApiError(error), 'Gagal Menghapus Program');
            setDeleting(false);
        }
    };
        ...
        return (
            <div>
                <div className="bg-card rounded-xl shadow-sm border border-border p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-card-foreground mb-2">Program tidak ditemukan</h3>
                    <button onClick={() => router.back()}
                        className="inline-flex items-center gap-2 text-orange-600 hover:text-orange-700 font-medium mt-2">
        ...
        <div className="space-y-6">
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500 via-orange-600 to-red-600 p-6">
                ...
                <div className="relative z-10 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-white">Edit Program HCDP</h1>
                            <p className="text-amber-100 text-sm">Perbarui informasi program</p>
                        </div>
                    </div>
                    <button
            ...
            {program && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                        { label: 'Status', value: formData.status.charAt(0).toUpperCase() + formData.status.slice(1), icon: Eye, color: statusStyles[formData.status]?.split(' ')[0] || 'text-blue-600', bg: statusStyles[formData.status]?.split(' ')[1] || 'bg-blue-50' },
                        { label: 'Kategori', value: formData.category, icon: BookOpen, color: 'text-amber-600', bg: 'bg-amber-50' },
                        { label: 'Level', value: formData.level.charAt(0).toUpperCase() + formData.level.slice(1), icon: UserCheck, color: 'text-emerald-600', bg: 'bg-emerald-50' },
                        { label: 'Dibuat', value: new Date(program.created_at).toLocaleDateString('id-ID'), icon: Calendar, color: 'text-purple-600', bg: 'bg-purple-50' },
                    ].map((stat, i) => (
                            ...
                            <div className="flex flex-col sm:flex-row gap-5">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-3">
                                        {formData.category && (
                                            <Badge className="bg-amber-100 text-amber-700 dark:text-amber-300 border-0">{formData.category}</Badge>
                                        )}
                                        <Badge className="bg-emerald-100 text-emerald-700 border-0 capitalize">{formData.level}</Badge>
                                        <Badge className={
