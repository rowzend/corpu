...
import { getJenisKompetensiList, createJenisKompetensi, updateJenisKompetensi, deleteJenisKompetensi, type JenisKompetensi, type JenisKompetensiPayload } from '@/lib/api/idp';
import { handleApiError } from '@/lib/api';
import { showSuccess, showError, showDeleteConfirm, showLoading, closeLoading } from '@/lib/sweetalert';
import { useThemeColors } from '@/lib/hooks/useThemeColors';
import { usePermission } from '@/lib/hooks/usePermission';

export default function JenisKompetensiPage() {
    ...
    const [items, setItems] = useState<JenisKompetensi[]>([]);
    ...
    const [editingItem, setEditingItem] = useState<JenisKompetensi | null>(null);
    const [formData, setFormData] = useState<Partial<JenisKompetensi>>({});
    const [isSubmitting, setIsSubmitting] = useState(false);

    const loadData = async () => {
        try {
            setIsLoading(true);
            const res = await getJenisKompetensiList(filters);
            setItems(res.data || []);
    ...
    const openEdit = (item: JenisKompetensi) => {
        setEditingItem(item);
        setFormData({ ...item });
        setShowForm(true);
    };

    const handleDelete = async (item: JenisKompetensi) => {
        const confirmed = await showDeleteConfirm(item.nama, 'Yakin ingin menghapus jenis kompetensi ini?');
        if (!confirmed) return;
        try {
            showLoading('Menghapus...');
            await deleteJenisKompetensi(item.id);
            closeLoading();
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (editingItem) {
                await updateJenisKompetensi(editingItem.id, formData as Partial<JenisKompetensiPayload>);
            } else {
                await createJenisKompetensi(formData as JenisKompetensiPayload);
            }
            setShowForm(false);
            closeLoading();
        ...
        <div className="space-y-6">
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-teal-600 via-teal-700 to-cyan-800 p-8">
                ...
                <div className="relative z-10 flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-white">Jenis Kompetensi</h1>
                                <p className="text-teal-100 text-sm">Kelola master data jenis kompetensi IDP (Teknis, Manajerial, Sosial Kultural, dll)</p>
                            </div>
                        </div>
                    </div>
                    {canCreate && (
                    <button onClick={openCreate}
                        ...
                        <Plus className="w-4 h-4" />Tambah Jenis Kompetensi
                    </button>
                    )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 mt-6">
                    {[
                        { label: 'Total', value: pagination.total, icon: Trophy, color: 'bg-teal-400/20 text-teal-200' },
                        { label: 'Aktif', value: items.filter(i => i.is_active).length, icon: Trophy, color: 'bg-green-400/20 text-green-200' },
                        { label: 'Halaman', value: pagination.total_pages, icon: Trophy, color: 'bg-cyan-400/20 text-cyan-200' },
                    ].map((stat, i) => (
            ...
            <div className={`${card.bgClass} ${card.borderClass} border rounded-xl shadow-sm p-4`}>
                <div className="flex items-center gap-3">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input type="text" placeholder="Cari jenis kompetensi..." value={filters.search || ''}
                ...
                <table className="w-full">
                    <thead>
                        <tr className={`border-b ${card.borderClass} ${text.mutedClass} text-xs uppercase tracking-wider`}>
                            ...
                            <th className="text-left py-4 px-4 font-medium">Nama Jenis Kompetensi</th>
            ...
            {showForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className={`relative ${card.bgClass} rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6`}>
                        <h2 className={`text-lg font-bold mb-6 ${text.primaryClass}`}>{editingItem ? 'Edit Jenis Kompetensi' : 'Tambah Jenis Kompetensi'}</h2>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="col-span-2 sm:col-span-1">
                                ...
                                <input type="text" value={formData.kode || ''} onChange={e => setFormData(p => ({ ...p, kode: e.target.value }))}
                                    className={`w-full px-3 py-2 border ${card.bgClass} ${card.borderClass} rounded-xl text-sm ${text.primaryClass} placeholder:text-muted-foreground focus:ring-2 focus:ring-teal-500`} placeholder="Contoh: TEKNIS" />
                            </div>
                            <div className="col-span-2 sm:col-span-1">
                            ...
                            <div className="col-span-2">
                                <label className={`block text-sm font-medium mb-1 ${text.primaryClass}`}>Nama Jenis Kompetensi <span className="text-red-500">*</span></label>
                            ...
                            <div className="col-span-2">
                                ...
                                <textarea value={formData.deskripsi || ''} onChange={e => setFormData(p => ({ ...p, deskripsi: e.target.value }))}
                                    className={`w-full px-3 py-2 border ${card.bgClass} ${card.borderClass} rounded-xl text-sm ${text.primaryClass} placeholder:text-muted-foreground focus:ring-2 focus:ring-teal-500`} rows={3} placeholder="Deskripsi jenis kompetensi" />
                            </div>
                        </div>
