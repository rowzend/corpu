...
import { getJenisKegiatanPengembanganList, createJenisKegiatanPengembangan, updateJenisKegiatanPengembangan, deleteJenisKegiatanPengembangan, getPilarPengembanganList, type JenisKegiatanPengembangan, type JenisKegiatanPengembanganPayload, type PilarPengembangan } from '@/lib/api/idp';
import { handleApiError } from '@/lib/api';
import { showSuccess, showError, showDeleteConfirm, showLoading, closeLoading } from '@/lib/sweetalert';
import { useThemeColors } from '@/lib/hooks/useThemeColors';
import { usePermission } from '@/lib/hooks/usePermission';

export default function JenisKegiatanPengembanganPage() {
    ...
    const [items, setItems] = useState<JenisKegiatanPengembangan[]>([]);
    const [pilarList, setPilarList] = useState<PilarPengembangan[]>([]);
    const [isLoading, setIsLoading] = useState(true);

    const [filters, setFilters] = useState({ page: 1, per_page: 20, search: '', is_active: '', pilar_pengembangan_id: '' });
    const [pagination, setPagination] = useState({ page: 1, per_page: 20, total: 0, total_pages: 0 });

    const [showForm, setShowForm] = useState(false);
    const [editingItem, setEditingItem] = useState<JenisKegiatanPengembangan | null>(null);
    const [formData, setFormData] = useState<Partial<JenisKegiatanPengembangan>>({});
    ...
    const loadData = async () => {
        try {
            setIsLoading(true);
            const res = await getJenisKegiatanPengembanganList(filters);
            setItems(res.data || []);
    ...
    const openEdit = (item: JenisKegiatanPengembangan) => {
        setEditingItem(item);
        setFormData({ ...item, pilar_pengembangan_id: item.pilar_pengembangan_id });
        setShowForm(true);
    };

    const handleDelete = async (item: JenisKegiatanPengembangan) => {
        const confirmed = await showDeleteConfirm(item.nama, 'Yakin ingin menghapus bentuk pengembangan kompetensi ini?');
        if (!confirmed) return;
        try {
            showLoading('Menghapus...');
            await deleteJenisKegiatanPengembangan(item.id);
            closeLoading();
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (editingItem) {
                await updateJenisKegiatanPengembangan(editingItem.id, formData as Partial<JenisKegiatanPengembanganPayload>);
            } else {
                await createJenisKegiatanPengembangan(formData as JenisKegiatanPengembanganPayload);
            }
            setShowForm(false);
            closeLoading();
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-teal-600 via-teal-700 to-cyan-800 p-8">
                ...
                <div className="relative z-10 flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-white">Bentuk Pengembangan Kompetensi</h1>
                                <p className="text-teal-100 text-sm">Kelola bentuk pengembangan kompetensi berdasarkan metode (model 70-20-10)</p>
                            </div>
                        </div>
                    </div>
                    {canCreate && (
                    <button onClick={openCreate}
                        ...
                        <Plus className="w-4 h-4" />Tambah Bentuk Pengembangan
                    </button>
                    )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 mt-6">
                    {[
                        ...
                        { label: 'Metode Terdaftar', value: pilarList.filter(p => p.is_active).length, icon: ListChecks, color: 'bg-cyan-400/20 text-cyan-200' },
                    ].map((stat, i) => (
            ...
            <div className={`${card.bgClass} ${card.borderClass} border rounded-xl shadow-sm p-4`}>
                <div className="flex items-center gap-3 flex-wrap">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input type="text" placeholder="Cari bentuk pengembangan..." value={filters.search || ''}
                    ...
                    <select value={filters.pilar_pengembangan_id} onChange={(e) => setFilters(prev => ({ ...prev, pilar_pengembangan_id: e.target.value, page: 1 }))}
                ...
                <table className="w-full">
                    <thead>
                        <tr className={`border-b ${card.borderClass} ${text.mutedClass} text-xs uppercase tracking-wider`}>
                            <th className="text-left py-4 px-4 font-medium">Metode Pengembangan Kompetensi</th>
                        ...
                        {isLoading ? (
                            ...
                            <tr key={item.id} className={`hover:bg-muted/50 transition-colors ${text.primaryClass}`}>
                                <td className="py-3 px-4">
                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${pilarBadge(item.pilar_pengembangan_nama, item.pilar_pengembangan_persentase)}`}>
                                        {item.pilar_pengembangan_nama} ({item.pilar_pengembangan_persentase}%)
                                    </span>
                                </td>
            ...
            {showForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className={`relative ${card.bgClass} rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6`}>
                        <h2 className={`text-lg font-bold mb-6 ${text.primaryClass}`}>{editingItem ? 'Edit Bentuk Pengembangan Kompetensi' : 'Tambah Bentuk Pengembangan Kompetensi'}</h2>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="col-span-2">
                                ...
                                <select value={formData.pilar_pengembangan_id ?? ''} onChange={e => setFormData(p => ({ ...p, pilar_pengembangan_id: parseInt(e.target.value) || undefined }))}
                            ...
                            <div className="col-span-2">
                                <label className={`block text-sm font-medium mb-1 ${text.primaryClass}`}>Bentuk Pengembangan <span className="text-red-500">*</span></label>
                                <input type="text" value={formData.nama || ''} onChange={e => setFormData(p => ({ ...p, nama: e.target.value }))}
                                    className={`w-full px-3 py-2 border ${card.bgClass} ${card.borderClass} rounded-xl text-sm ${text.primaryClass} placeholder:text-muted-foreground focus:ring-2 focus:ring-teal-500`} placeholder="Contoh: Bimtek/Diklat eksternal" />
                            </div>
                            <div className="col-span-2">
