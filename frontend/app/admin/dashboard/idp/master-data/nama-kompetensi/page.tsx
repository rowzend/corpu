...
import { getNamaKompetensiList, createNamaKompetensi, updateNamaKompetensi, deleteNamaKompetensi, getJenisKompetensiList, type NamaKompetensi, type NamaKompetensiPayload, type JenisKompetensi } from '@/lib/api/idp';
import { handleApiError } from '@/lib/api';
import { showSuccess, showError, showDeleteConfirm, showLoading, closeLoading } from '@/lib/sweetalert';
import { useThemeColors } from '@/lib/hooks/useThemeColors';
import { usePermission } from '@/lib/hooks/usePermission';

export default function NamaKompetensiPage() {
    ...
    const [items, setItems] = useState<NamaKompetensi[]>([]);
    const [jenisList, setJenisList] = useState<JenisKompetensi[]>([]);
    const [isLoading, setIsLoading] = useState(true);

    const [filters, setFilters] = useState({ page: 1, per_page: 20, search: '', is_active: '', jenis_kompetensi_id: '' });
    const [pagination, setPagination] = useState({ page: 1, per_page: 20, total: 0, total_pages: 0 });

    const [showForm, setShowForm] = useState(false);
    const [editingItem, setEditingItem] = useState<NamaKompetensi | null>(null);
    const [formData, setFormData] = useState<Partial<NamaKompetensi>>({});
    ...
    const loadData = async () => {
        try {
            setIsLoading(true);
            const res = await getNamaKompetensiList(filters);
            setItems(res.data || []);
    ...
    const openCreate = () => {
        setEditingItem(null);
        setFormData({ is_active: true, jenis_kompetensi_id: filters.jenis_kompetensi_id ? Number(filters.jenis_kompetensi_id) : undefined });
        setShowForm(true);
    };

    const openEdit = (item: NamaKompetensi) => {
        setEditingItem(item);
        setFormData({ ...item });
        setShowForm(true);
    };

    const handleDelete = async (item: NamaKompetensi) => {
        const confirmed = await showDeleteConfirm(item.nama, 'Yakin ingin menghapus nama kompetensi ini?');
        if (!confirmed) return;
        try {
            showLoading('Menghapus...');
            await deleteNamaKompetensi(item.id);
            closeLoading();
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (editingItem) {
                await updateNamaKompetensi(editingItem.id, formData as Partial<NamaKompetensiPayload>);
            } else {
                await createNamaKompetensi(formData as NamaKompetensiPayload);
            }
            setShowForm(false);
            closeLoading();
        ...
        <div className="space-y-6">
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-teal-600 via-teal-700 to-cyan-800 p-8">
                ...
                <div className="relative z-10 flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-white">Nama Kompetensi</h1>
                                <p className="text-teal-100 text-sm">Kelola master data nama kompetensi IDP, terkait dengan jenis kompetensi</p>
                            </div>
                        </div>
                    </div>
                    {canCreate && (
                    <button onClick={openCreate}
                        ...
                        <Plus className="w-4 h-4" />Tambah Nama Kompetensi
                    </button>
                    )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 mt-6">
                    {[
                        { label: 'Total', value: pagination.total, icon: Lightbulb, color: 'bg-teal-400/20 text-teal-200' },
                        { label: 'Aktif', value: items.filter(i => i.is_active).length, icon: Lightbulb, color: 'bg-green-400/20 text-green-200' },
                        { label: 'Halaman', value: pagination.total_pages, icon: Lightbulb, color: 'bg-cyan-400/20 text-cyan-200' },
                    ].map((stat, i) => (
            ...
            <div className={`${card.bgClass} ${card.borderClass} border rounded-xl shadow-sm p-4`}>
                <div className="flex items-center gap-3">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input type="text" placeholder="Cari nama kompetensi..." value={filters.search || ''}
                    ...
                    <select value={filters.jenis_kompetensi_id} onChange={(e) => setFilters(prev => ({ ...prev, jenis_kompetensi_id: e.target.value, page: 1 }))}
                ...
                <table className="w-full">
                    <thead>
                        <tr className={`border-b ${card.borderClass} ${text.mutedClass} text-xs uppercase tracking-wider`}>
                            ...
                            <th className="text-left py-4 px-4 font-medium">Nama Kompetensi</th>
                            <th className="text-left py-4 px-4 font-medium">Jenis Kompetensi</th>
                        ...
                        {isLoading ? (
                            ...
                            <tr key={item.id} className={`hover:bg-muted/50 transition-colors ${text.primaryClass}`}>
                                <td className="py-3 px-4">
                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-100 text-teal-800">{item.kode || '-'}</span>
                                </td>
                                <td className="py-3 px-4">
                                    <div className="font-medium text-sm">{item.nama}</div>
                                </td>
                                <td className="py-3 px-4">
                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${text.mutedClass} bg-muted`}>{item.jenis_kompetensi_nama || '-'}</span>
                                </td>
            ...
            {showForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className={`relative ${card.bgClass} rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6`}>
                        <h2 className={`text-lg font-bold mb-6 ${text.primaryClass}`}>{editingItem ? 'Edit Nama Kompetensi' : 'Tambah Nama Kompetensi'}</h2>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="col-span-2">
                                <label className={`block text-sm font-medium mb-1 ${text.primaryClass}`}>Jenis Kompetensi <span className="text-red-500">*</span></label>
                                <select value={formData.jenis_kompetensi_id ?? ''} onChange={e => setFormData(p => ({ ...p, jenis_kompetensi_id: e.target.value ? Number(e.target.value) : undefined }))}
                            ...
                            <div className="col-span-2 sm:col-span-1">
                                <label className={`block text-sm font-medium mb-1 ${text.primaryClass}`}>Kode</label>
                                <input type="text" value={formData.kode || ''} onChange={e => setFormData(p => ({ ...p, kode: e.target.value }))}
                                    className={`w-full px-3 py-2 border ${card.bgClass} ${card.borderClass} rounded-xl text-sm ${text.primaryClass} placeholder:text-muted-foreground focus:ring-2 focus:ring-teal-500`} placeholder="Contoh: TEKNIS-01" />
                            </div>
                            <div className="col-span-2 sm:col-span-1">
                            ...
                            <div className="col-span-2">
                                <label className={`block text-sm font-medium mb-1 ${text.primaryClass}`}>Nama Kompetensi <span className="text-red-500">*</span></label>
                                <input type="text" value={formData.nama || ''} onChange={e => setFormData(p => ({ ...p, nama: e.target.value }))}
                                    className={`w-full px-3 py-2 border ${card.bgClass} ${card.borderClass} rounded-xl text-sm ${text.primaryClass} placeholder:text-muted-foreground focus:ring-2 focus:ring-teal-500`} placeholder="Contoh: Penguasaan Teknologi Informasi" />
                            </div>
                            <div className="col-span-2">
                                ...
                                <textarea value={formData.deskripsi || ''} onChange={e => setFormData(p => ({ ...p, deskripsi: e.target.value }))}
                                    className={`w-full px-3 py-2 border ${card.bgClass} ${card.borderClass} rounded-xl text-sm ${text.primaryClass} placeholder:text-muted-foreground focus:ring-2 focus:ring-teal-500`} rows={3} placeholder="Deskripsi nama kompetensi" />
                            </div>
                        </div>
