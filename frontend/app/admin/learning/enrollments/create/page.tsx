...
export default function CreateEnrollmentPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            router.push('/admin/learning/enrollments');
        } catch (error) {
            ...
            <form onSubmit={handleSubmit}>
                <div className="bg-card rounded-xl shadow-sm border border-border">
                    <div className="px-6 py-4 border-b border-border">
                        <h2 className="text-lg font-semibold text-card-foreground">Form Enrollment</h2>
                    </div>
                    <div className="p-6 space-y-6">
                        <div className="space-y-2">
                            <Label htmlFor="course" className="text-sm font-medium text-card-foreground">Pilih Kursus <span className="text-red-500">*</span></Label>
                            <select id="course" value={formData.course}
                                onChange={(e) => setFormData(prev => ({ ...prev, course: e.target.value }))}
                                disabled={loadingCourses}
                                className="w-full px-3 py-2.5 border border-border rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-muted text-sm">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="username" className="text-sm font-medium text-card-foreground">Username Siswa <span className="text-red-500">*</span></Label>
                            <div className="relative">
                                ...
                                <input id="username"
                                    ...
                                    className="w-full pl-10 pr-4 py-2.5 border border-border rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-muted focus:bg-card transition-colors text-sm" required />
                            </div>
                            <p className="text-xs text-muted-foreground">Masukkan username yang terdaftar di sistem</p>
                        </div>
                    </div>
                </div>
