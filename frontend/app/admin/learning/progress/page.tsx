    ...
    if (loading) {
        return (
            <div className="space-y-6">
                ...
                <div className="space-y-3">{[1, 2, 3].map(i => <div key={i} className="h-20 bg-muted rounded-xl animate-pulse"></div>)}</div>
            </div>
        );
    }
            ...
            {filtered.length > 0 ? (
                <div className="space-y-3">
                    {filtered.map((enrollment) => (
                        <div key={enrollment.id} className="bg-card rounded-xl shadow-sm border border-border hover:shadow-md hover:border-teal-100 transition-all">
                            <div className="p-4">
                ...
                <div className="bg-card rounded-xl shadow-sm border border-border p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-card-foreground mb-2">Belum ada data progres</h3>
                    <p className="text-muted-foreground">Data akan muncul setelah siswa terdaftar di kursus</p>
                </div>
            )}
        </div>
    );
