    ...
    if (loading) {
        return (
            <div className="space-y-6">
                ...
                <div className="space-y-3">{[1, 2, 3].map(i => <div key={i} className="h-28 bg-gray-200 rounded-xl animate-pulse"></div>)}</div>
            </div>
        );
    }
        ...
        <div className="space-y-6">
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-pink-600 via-pink-700 to-rose-800 p-8">
                ...
                <div className="relative z-10">
                    <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => router.push('/learning/quizzes/create')}
                            className="inline-flex items-center gap-2 bg-white text-pink-700 hover:bg-pink-50 px-5 py-2.5 rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                <div className="relative">
                    ...
                    <input type="text" placeholder="Cari quiz..." value={search}
                        ...
                        className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-pink-500 focus:border-pink-500 bg-gray-50 focus:bg-white transition-colors text-sm" />
                </div>
            </div>
            ...
            {filtered.length > 0 ? (
                <div className="grid gap-4">
                    {filtered.map((quiz) => (
                        <div key={quiz.id} className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md hover:border-pink-100 transition-all">
                            <div className="p-5">
                                <div className="flex items-start justify-between">
                                    <div className="flex-1 min-w-0">
                                        <h3 className="font-semibold text-gray-900 mb-1">{quiz.title}</h3>
                                        <p className="text-sm text-gray-500 mb-3">Pelajaran: {quiz.lesson_title || `ID: ${quiz.lesson}`}</p>
                                    ...
                                    <div className="flex gap-2 flex-shrink-0 ml-4">
                                        <button onClick={() => router.push(`/learning/quizzes/${quiz.id}/questions`)}
                                            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 hover:text-pink-600 hover:bg-pink-50 rounded-lg transition-colors">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">{search ? 'Quiz tidak ditemukan' : 'Belum ada quiz'}</h3>
                    <p className="text-gray-500 mb-6">{search ? 'Coba ubah kata kunci pencarian Anda' : 'Mulai dengan membuat quiz pertama Anda'}</p>
                    {!search && (
                        <button onClick={() => router.push('/learning/quizzes/create')}
