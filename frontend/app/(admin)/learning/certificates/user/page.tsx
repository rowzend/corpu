            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                <div className="relative">
                    ...
                    <input type="text" placeholder="Cari nama, username, kursus, nomor sertifikat..." value={search}
                        ...
                        className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-violet-500 bg-gray-50 focus:bg-white transition-colors text-sm" />
                </div>
            </div>
            ...
            {Object.keys(grouped).length > 0 ? (
                <div className="space-y-6">
                    {Object.entries(grouped).map(([userName, certs]) => (
                        <div key={userName} className="bg-white rounded-xl shadow-sm border border-gray-100">
                            ...
                            <div className="p-4 space-y-2">
                                {certs.map(cert => (
                                    <div key={cert.id} className="flex items-center justify-between p-3 border border-gray-100 rounded-xl hover:bg-gray-50 hover:border-violet-100 transition-all group">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 mt-1">
                                                    ...
                                                    {cert.user_username && <span className="text-gray-400">@{cert.user_username}</span>}
                                                </div>
                                            </div>
                                        </div>
                                        <button onClick={() => handleDownload(cert.id)}
                                            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 hover:text-violet-600 hover:bg-violet-50 rounded-lg transition-colors flex-shrink-0 ml-3">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
                    ...
                    <p className="text-gray-500 text-sm">Sertifikat akan muncul setelah pengguna menyelesaikan kursus</p>
                </div>
            )}
        </div>
    );
