            ...
            {filtered.length > 0 ? (
                <div className="grid gap-4">
                    {filtered.map((course) => {
                        ...
                        return (
                            <div key={course.id} className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md hover:border-emerald-100 transition-all group">
                                <div className="p-5">
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm text-gray-500 line-clamp-1 mb-3">{course.short_description || course.description}</p>
                                            <div className="flex items-center gap-4 text-sm">
                                                <span className="flex items-center gap-1.5"><Layers className="w-4 h-4 text-emerald-500" /><span className="font-medium">{totalModules}</span><span className="text-gray-500">Modul</span></span>
                                                <span className="flex items-center gap-1.5"><BookOpen className="w-4 h-4 text-blue-500" /><span className="font-medium">{totalLessons}</span><span className="text-gray-500">Pelajaran</span></span>
                                                <span className="text-gray-400 text-xs"><Clock className="w-3 h-3 inline mr-1" />{course.level} · {course.duration_minutes} menit</span>
                                            </div>
                                        </div>
                                        <div className="flex gap-2 flex-shrink-0 ml-4">
                                            ...
                                            <button onClick={() => router.push(`/courses/${course.slug}`)}
                                                className="inline-flex items-center gap-1.5 px-3 py-2 text-sm text-gray-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
                                                Lihat
                                            </button>
                                        </div>
                                    </div>
