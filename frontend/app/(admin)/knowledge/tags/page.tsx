                ...
                <div className="relative z-10">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                                <h1 className="text-2xl font-bold text-white">Tags Knowledge Base</h1>
                                <p className="text-orange-100 text-sm">Kelola tags untuk artikel knowledge base</p>
                            </div>
                        </div>
                        <button
                            onClick={() => router.push('/knowledge/tags/create')}
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-6">
                        {[
                            { label: 'Total Tags', value: tags.length, icon: TagIcon, color: 'bg-orange-400/20 text-orange-200' },
                            { label: 'Aktif', value: tags.filter(t => t.is_active).length, icon: Palette, color: 'bg-green-400/20 text-green-200' },
                            { label: 'Total Artikel', value: tags.reduce((sum, tag) => sum + (tag.article_count || 0), 0), icon: Hash, color: 'bg-blue-400/20 text-blue-200' },
                        ].map((stat, i) => (
            ...
            {filteredTags.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredTags.map((tag) => (
                        <div
                            key={tag.id}
                            className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-orange-100 transition-all group"
                        >
                            <div className="p-5">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">
                        {searchTerm ? 'Tidak ada tag ditemukan' : 'Belum ada tag'}
                    </h3>
