...
class Command(BaseCommand):
    help = "Import/sync Mr_assessment from Laravel table Mr_assessment (DATABASES['laravel']) into Django DB (pegawai.MrAssessment)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_assessment into pegawai.MrAssessment'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_assessment'
        ...
        model_fields = {f.name for f in MrAssessment._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrAssessment.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_assessment completed'))
