...
from apps.pegawai.models import MrDiklatFungsional


class Command(BaseCommand):
    help = "Import/sync Mr_diklat_fungsional from Laravel table Mr_diklat_fungsional (DATABASES['laravel']) into Django DB (pegawai.MrDiklatFungsional)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_diklat_fungsional into pegawai.MrDiklatFungsional'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_diklat_fungsional'
        ...
        model_fields = {f.name for f in MrDiklatFungsional._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrDiklatFungsional.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_diklat_fungsional completed'))
