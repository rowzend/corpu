...
from apps.pegawai.models import MrDiklatStruktural


class Command(BaseCommand):
    help = "Import/sync Mr_diklat_struktural from Laravel table Mr_diklat_struktural (DATABASES['laravel']) into Django DB (pegawai.MrDiklatStruktural)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_diklat_struktural into pegawai.MrDiklatStruktural'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_diklat_struktural'
        ...
        model_fields = {f.name for f in MrDiklatStruktural._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrDiklatStruktural.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_diklat_struktural completed'))
