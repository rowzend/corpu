...
from apps.pegawai.models import MrTugasLn


class Command(BaseCommand):
    help = "Import/sync Mr_tugas_ln from Laravel table Mr_tugas_ln (DATABASES['laravel']) into Django DB (pegawai.MrTugasLn)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_tugas_ln into pegawai.MrTugasLn'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_tugas_ln'
        ...
        model_fields = {f.name for f in MrTugasLn._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrTugasLn.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_tugas_ln completed'))
