...
from apps.pegawai.models import MrSkp


class Command(BaseCommand):
    help = "Import/sync Mr_skp from Laravel table Mr_skp (DATABASES['laravel']) into Django DB (pegawai.MrSkp)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_skp into pegawai.MrSkp'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_skp'
        ...
        model_fields = {f.name for f in MrSkp._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrSkp.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_skp completed'))
