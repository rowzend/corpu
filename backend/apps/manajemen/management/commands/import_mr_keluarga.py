...
from apps.pegawai.models import MrKeluarga


class Command(BaseCommand):
    help = "Import/sync Mr_keluarga (pasangan+anak) from Laravel table Mr_keluarga (DATABASES['laravel']) into Django DB (pegawai.MrKeluarga)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_keluarga into pegawai.MrKeluarga'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_keluarga'
        ...
        model_fields = {f.name for f in MrKeluarga._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrKeluarga.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_keluarga completed'))
