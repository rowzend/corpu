...
from apps.pegawai.models import MrAngkaKredit


class Command(BaseCommand):
    help = "Import/sync Mr_angka_kredit from Laravel table Mr_angka_kredit (DATABASES['laravel']) into Django DB (pegawai.MrAngkaKredit)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_angka_kredit into pegawai.MrAngkaKredit'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_angka_kredit'
        ...
        model_fields = {f.name for f in MrAngkaKredit._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrAngkaKredit.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_angka_kredit completed'))
