...
from apps.pegawai.models import MsTataNaskahPegawai


class Command(BaseCommand):
    help = "Import/sync Ms_tata_naskah_pegawai from Laravel DB (DATABASES['laravel']) into Django DB (pegawai.MsTataNaskahPegawai)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing ms_tata_naskah_pegawai into pegawai.MsTataNaskahPegawai'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM ms_tata_naskah_pegawai'
        ...
        model_fields = {f.name for f in MsTataNaskahPegawai._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MsTataNaskahPegawai.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import ms_tata_naskah_pegawai completed'))
