...
from apps.pegawai.models import MrDiklatTeknis


class Command(BaseCommand):
    help = "Import/sync Mr_diklat_teknis from Laravel table Mr_diklat_teknis (DATABASES['laravel']) into Django DB (pegawai.MrDiklatTeknis)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_diklat_teknis into pegawai.MrDiklatTeknis'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_diklat_teknis'
        ...
        model_fields = {f.name for f in MrDiklatTeknis._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrDiklatTeknis.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_diklat_teknis completed'))
