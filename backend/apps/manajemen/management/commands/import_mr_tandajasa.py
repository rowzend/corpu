...
from apps.pegawai.models import MrTandaJasa


class Command(BaseCommand):
    help = "Import/sync Mr_tandajasa from Laravel table Mr_tandajasa (DATABASES['laravel']) into Django DB (pegawai.MrTandaJasa)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_tandajasa into pegawai.MrTandaJasa'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_tandajasa'
        ...
        model_fields = {f.name for f in MrTandaJasa._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrTandaJasa.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_tandajasa completed'))
