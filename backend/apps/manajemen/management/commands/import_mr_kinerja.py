...
from apps.pegawai.models import MrKinerja


class Command(BaseCommand):
    help = "Import/sync Mr_kinerja from Laravel table Mr_kinerja (DATABASES['laravel']) into Django DB (pegawai.MrKinerja)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_kinerja into pegawai.MrKinerja'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_kinerja'
            cursor.execute(sql)
        ...
        model_fields = {f.name for f in MrKinerja._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrKinerja.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_kinerja completed'))
