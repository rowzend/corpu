...
from apps.pegawai.models import MrSeminar


class Command(BaseCommand):
    help = "Import/sync Mr_seminar from Laravel table Mr_seminar (DATABASES['laravel']) into Django DB (pegawai.MrSeminar)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('⬇️  Importing Mr_seminar into pegawai.MrSeminar'))
        self.stdout.write('=' * 70)

        with connections['laravel'].cursor() as cursor:
            sql = 'SELECT * FROM Mr_seminar'
        ...
        model_fields = {f.name for f in MrSeminar._meta.fields}
        ...
        for row in rows:
            ...
            _, was_created = MrSeminar.objects.update_or_create(
                id=int(rid),
                defaults=defaults,
            )
        ...
        self.stdout.write(self.style.SUCCESS('✅ Import Mr_seminar completed'))
