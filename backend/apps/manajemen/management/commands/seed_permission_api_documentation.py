...
class Command(BaseCommand):
    help = 'Seed Permission Module/Control/Function/Rules for API Documentation (read-only); assign to Super Admin groups'

    def handle(self, *args, **options):
        self.stdout.write('=' * 70)
        self.stdout.write(self.style.SUCCESS('🌱 Seeding Permissions: Pengaturan → Dokumentasi API'))
        self.stdout.write('=' * 70)
        ...
        rules = PermissionRule.objects.filter(module=module, control=control_objs['api_documentation'], is_active=True)
        assigned = 0
