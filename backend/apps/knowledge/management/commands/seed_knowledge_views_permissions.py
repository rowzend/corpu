"""
Seed Knowledge Base Views Permissions
Creates permissions for view management

Usage:
  python manage.py seed_knowledge_views_permissions
"""
...
class Command(BaseCommand):
    help = 'Seed Knowledge Base Views Permissions'

    def handle(self, *args, **options):
        self.stdout.write('=' * 70)
        self.stdout.write(self.style.SUCCESS('🌱 Seeding Knowledge Base Views Permissions'))
        self.stdout.write('=' * 70)
        ...
        # 2. Create control for views
        view_control, created = PermissionControl.objects.get_or_create(
            nama_kontrol='view',
            defaults={
                'label_kontrol': 'Views',
                'deskripsi_kontrol': 'Analisis dan tracking view artikel'
            }
        )
        
        if created:
            self.stdout.write(f'  ✅ Created control: {view_control.label_kontrol}')
        else:
            self.stdout.write(f'  ♻️  Using existing control: {view_control.label_kontrol}')
        ...
        for func_name, func in functions.items():
            ...
            if created:
                rules_created += 1
                self.stdout.write(f'  ✅ Created rule: knowledge.view.{func_name}')
            else:
                rules_existing += 1
                self.stdout.write(f'  ♻️  Rule already exists: knowledge.view.{func_name}')
        ...
        try:
            ...
            # Get all view rules
            view_rules = PermissionRule.objects.filter(
                module=knowledge_module,
                control=view_control
            )
            ...
            for rule in view_rules:
                ...
                if created:
                    assigned_count += 1
                    self.stdout.write(f'  ✅ Assigned to Super Admin: knowledge.view.{rule.function.nama_fungsi}')
        ...
        # Summary
        self.stdout.write('')
        self.stdout.write(self.style.SUCCESS('✅ Knowledge Base Views permissions seeded successfully!'))
